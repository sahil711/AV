...
class DummyEncoding(Encoding):
    ...
    def create_encoding_dict(self, X, y):
        ...
        return {col: [k for k, v in X[col].astype('object').value_counts().to_dict().iteritems() if v>self.threshold]
                for col in self.categorical_columns}

    def apply_encoding(self, X_in, encoding_dict):
        X = self.convert_input(X_in)
        for col, values in encoding_dict.iteritems():
            for val in values:
...
class TargetEncoding(Encoding):
    ...
    def create_encoding_dict(self, X, y):
        ...
        for col in self.categorical_columns:
            ...
            averages = pd.DataFrame(zip(category_values, y)).groupby(0)[1].agg(["mean", "count"]).to_dict(orient='index')
            enc_d = {}
